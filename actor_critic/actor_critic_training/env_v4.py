...
class Env_tsp():
	...
	def get_batch_nodes(self, n_samples, seed = None):
		...
		if seed is not None:
			torch.manual_seed(seed)
		a,b,c = n_samples,self.city_t,2   #dimension of the pytorch tensor to be generated
		low,high = 0,1 #range of uniform distribution
		x = torch.distributions.uniform.Uniform(0,10).sample([a,b,c])
		return x.squeeze()
