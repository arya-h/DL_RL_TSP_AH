...
class TSPDistCost(TSPEnv):
    ...
    def _generate_coordinates(self):
        np.random.seed(1)
        xL = np.random.rand(self.N)
        yL = np.random.rand(self.N)
        #x = np.array(xL)
        #y = np.array(yL)

        return np.vstack([xL, yL])
    ...
    def render_custom(self, path):
        ...
        for _ in range(1, len(path)):
            #tour_l += self._get_node_distance(path[_],path[_-1])
    ...
    def render(self, mode="human"):
        ...
        #        dist += self.distance_matrix[self.path[i], self.path[i+1]]
        #    print(f"trip distance : {dist}")
        #    print(self.path)
        #fig, ax = plt.subplots(figsize=(12, 8))
        #ax.scatter(coords[0], coords[1], s=40)
